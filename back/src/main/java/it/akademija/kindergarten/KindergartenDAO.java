...
public interface KindergartenDAO extends JpaRepository<Kindergarten, String> {
	...
	@Query("SELECT new it.akademija.kindergarten.KindergartenInfo(k.id, k.name, k.address, k.elderate) FROM Application a JOIN Kindergarten k ON a.approvedKindergarten=k.id WHERE a.id=?1")
